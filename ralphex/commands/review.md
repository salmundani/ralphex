...
Execute an iterative review loop where Codex reviews your existing code, you (Claude Code) filter the suggestions and implement fixes, then Codex reviews again. Loop until Codex gives a clean review.

**User's focus area (optional):** $ARGUMENTS
...
- **Accept**: Real bugs, security issues, correctness problems, or clear improvements directly related to the code changes. These will be addressed in the next step.
...
- **Defer**: Valid observations that are out of scope for the current review (e.g., pre-existing issues, unrelated refactoring suggestions).
...
### If clean (LGTM):
- Inform the user that Codex approved the code
...
Implement the accepted corrections from Step 2:

1. For each accepted correction, explore the relevant code (Read, Grep, Glob) to understand the context.
2. Make the necessary code changes using Write/Edit.
3. Keep changes focused — only address the accepted corrections, do not refactor unrelated code.
4. Verify the changes are consistent with the rest of the codebase.
...
**IMPORTANT**: The workspace MUST be completely clean before proceeding back to the review step. Codex reviews the committed diff, so any uncommitted changes will be missed.

After committing, go back to **Step 1: Codex Code Review** for the next iteration.
...
- **The loop is fully automatic.** No user approval is needed between iterations. Only stop when Codex gives LGTM or all suggestions are rejected/deferred.
...
- **The workspace must be clean before the first review.** This command reviews existing committed code — uncommitted changes are not visible to Codex.
